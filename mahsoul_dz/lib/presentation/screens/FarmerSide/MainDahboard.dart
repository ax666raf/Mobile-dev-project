      ...
      builder: (context, authState) {
        ...
        if (farmerId == null) {
          return Scaffold(
            ...
            body: Center(
              child: Text(l10n.pleaseLoginToViewDashboard),
            ),
          );
        }
        ...
        return MultiBlocProvider(
          providers: [
            BlocProvider(create: (context) => FarmerDashboardCubit(DependencyInjection.farmerRepository, farmerId!)..loadDashboardData()),
            BlocProvider(create: (context) => FarmerOrderCubit(DependencyInjection.farmerRepository, farmerId!)..loadOrders()),
          ],
              ...
              return BlocBuilder<FarmerOrderCubit, FarmerOrderState>(
                builder: (context, orderState) {
                  ...
                  if (dashboardState is FarmerDashboardLoaded) {
                    ...
                    ordersToday = dashboard['total_orders'] as int? ?? 
                                  dashboard['orders_today'] as int? ?? 0;
                    totalEarningsValue = (dashboard['total_earnings'] as num?)?.toDouble() ?? 0.0;
                    pendingDeliveriesCount = dashboard['pending_deliveries'] as int? ?? 0;
                  }
                  ...
                  if (orderState is FarmerOrderLoaded) {
                    ...
                    if (totalEarningsValue == 0.0 && ordersData.isNotEmpty) {
                      totalEarningsValue = ordersData.fold<double>(0.0, (sum, order) {
                        final orderData = order as Map<String, dynamic>;
                        final price = (orderData['total_price'] as num?)?.toDouble() ?? 
                                     (orderData['totalPrice'] as num?)?.toDouble() ?? 0.0;
                        return sum + price;
                      });
                    }
                    ...
                    if (pendingDeliveriesCount == 0) {
                      pendingDeliveriesCount = ordersData.where((order) {
                        final status = (order as Map<String, dynamic>)['status'] as String? ?? '';
                    ...
                    recentOrders = ordersData.take(5).map((orderData) {
                      return _convertOrderFromBackend(orderData as Map<String, dynamic>, l10n);
                    }).toList();
                  }
                              ...
                              Row(
                                ...
                                children: [
                                  ...
                                  FarmerCard(
                                    ...
                                    value: '${totalEarningsValue.toStringAsFixed(0)} DA',
                                  ),
                                ],
                              ),
