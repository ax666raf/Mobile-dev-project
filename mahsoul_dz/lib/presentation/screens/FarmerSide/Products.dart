      ...
      builder: (context, authState) {
        ...
        if (farmerId == null) {
          ...
          return Scaffold(
            ...
            body: Center(
              child: Text(l10n.pleaseLoginToViewProducts),
            ),
          );
        }
        
        return BlocProvider(
          create: (context) => FarmerProductCubit(DependencyInjection.farmerRepository, farmerId!)..loadProducts(),
                    ...
                    children: [
                      ...
                      ProductType(
                        ...
                        onTap: () {
                          ...
                          context.read<FarmerProductCubit>().loadProductsByCategory('Vegetables');
                        },
                      ),
                      SizedBox(width: 8),
                      ...
                      ProductType(
                        ...
                        onTap: () {
                          ...
                          context.read<FarmerProductCubit>().loadProductsByCategory('Fruits');
                        },
                      ),
                      SizedBox(width: 8),
                      ...
                      ProductType(
                        ...
                        onTap: () {
                          ...
                          context.read<FarmerProductCubit>().loadProductsByCategory('Grains');
                        },
                      ),
                      SizedBox(width: 8),
                      ...
                      ProductType(
                        ...
                        isSelected: widget.selectedCategory == 'Out of stock',
                        onTap: () {
                    ...
                    if (state is FarmerProductLoaded) {
                      ...
                      List<Product> products = productsData.map((p) {
                        ...
                        if (weights.isNotEmpty) {
                          final firstWeightMap = weights.first as Map<String, dynamic>?;
                          if (firstWeightMap != null) {
                            firstWeight = firstWeightMap['weight_value'] as String? ?? '1kg';
                          }
                        }
                        // Calculate quantity from first weight (e.g., "1kg" -> 1.0, "500g" -> 0.5)
                        double quantity = 1.0;
                        if (firstWeight.toLowerCase().endsWith('kg')) {
                          final kgValue = firstWeight.toLowerCase().replaceAll('kg', '').trim();
                          quantity = double.tryParse(kgValue) ?? 1.0;
                        } else if (firstWeight.toLowerCase().endsWith('g')) {
                          final gValue = firstWeight.toLowerCase().replaceAll('g', '').trim();
                          quantity = (double.tryParse(gValue) ?? 1000.0) / 1000.0;
                        }
                      ...
                      final filteredProducts = filterProducts(widget.selectedCategory, products);
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final imagePath = productData['image_path'] as String? ?? 'lib/assets/IMAGE.png';
                          // Get weights for display
                          final weights = productData['weights'] as List<dynamic>? ?? [];
...
List<Product> filterProducts(String selectedCategory, List<Product> products) {
  if (selectedCategory == 'All') {
    ...
    return products.where((product) => product.status == 'out of stock').toList();
  } else {
    return products
        .where(
