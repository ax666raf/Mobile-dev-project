  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (args is Map && args['filter'] != null) {
        setState(() {
          selectedOrderType = args['filter'].toString();
        });
      }
    });
  }
      ...
      builder: (context, authState) {
        ...
        if (farmerId == null) {
          return Scaffold(
            ...
            body: Center(child: Text(l10n.pleaseLoginToViewOrders)),
          );
        }

        return BlocProvider(
          create: (context) =>
              FarmerOrderCubit(DependencyInjection.farmerRepository, farmerId!)
                ..loadOrders(),
          child: Scaffold(
            backgroundColor: Colors.white,
            appBar: AppBar(
...
List<Order> filterOrders(String selectedOrderType, List<Order> orders) {
  if (selectedOrderType == 'All') {
    ...
    return orders
        .where(
          (order) =>
              order.statusString.toLowerCase() ==
              selectedOrderType.toLowerCase(),
        )
        .toList();
  }
