...
import 'package:mahsoul_dz/presentation/screens/customerSide/order_confirmation_dialog.dart';
...
class OrderConfirmationPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(create: (context) => CartCubit(DependencyInjection.cartRepository, customerId)..loadCart()),
        BlocProvider(create: (context) => CustomerProfileCubit(DependencyInjection.profileRepository, customerId)..loadProfile()),
        BlocProvider(create: (context) => DeliveryAddressCubit(DependencyInjection.deliveryAddressRepository, customerId)..loadAddresses()),
        BlocProvider(create: (context) => OrderCubit(DependencyInjection.orderRepository, customerId)),
      ],
      child: Scaffold(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ...cartState.items.map((item) {
                        ...
                        final farmName = product?['farmer']?['farm_name'] as String? ?? '';
                        final quantity = item['quantity'] as int? ?? 1;
                        final weight = item['selected_weight'] as String? ?? '';
                        final price = (product?['price'] as num?)?.toDouble() ?? 0.0;
                        final imagePath = product?['image_path'] as String? ?? 'lib/assets/carrot.png';
                                ...
                                onPressed: isLoading ? null : () {
                                  ...
                                  if (addressState is DeliveryAddressLoaded) {
                                    final defaultAddress = addressState.addresses.firstWhere(
                                      ...
                                      orElse: () => addressState.addresses.isNotEmpty ? addressState.addresses.first : {},
                                    );
                                  ...
                                  orderCubit.placeOrder(
                                    ...
                                    paymentMethod: l10n.cash, // Default payment method
                                  );
                                },
  ...
  Widget _buildInfoRow(BuildContext context, IconData icon, String title, String value, AppLocalizations l10n, {VoidCallback? onEdit}) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Divider(thickness: 1),
          SizedBox(height: 12),
          _TotalRow(l10n.total, '${total.toStringAsFixed(2)} ${l10n.da}', isBold: true),
        ],
      ),
    );
  }
